...
{
  /// <summary>
  /// Interface used by <see cref="IdentityServicesManager{TUser, TDbContext}"/>
  ...
  public interface IIdentityServiceManager<TUser> where TUser : IdentityDbUser
  {
    ...
    Task<UserLoginResult> SignIn(string email, string password);
    ...
    Task<UserRegistrationResult> Register(TUser request);
  }
}
